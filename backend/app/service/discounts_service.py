...
async def create_discount(*, session: AsyncSession, discount_data: DiscountCreate) -> DiscountResponse:
    ...
    discount = await discounts_repository.create_discount(session=session, discount_data=DiscountCreate(**data))
...
async def get_discount(*, session: AsyncSession, discount_id: int) -> DiscountResponse:
    ...
    """
    discount = await discounts_repository.get_discount(session=session, discount_id=discount_id)
    if not discount:
...
async def get_discounts(*, session: AsyncSession) -> Page[DiscountResponse]:
    ...
    """
    query = discounts_repository.get_discounts_query()
...
async def update_discount(
    *, session: AsyncSession, discount_id: int, discount_data: DiscountUpdate
    ...
    """
    existing = await discounts_repository.get_discount(session=session, discount_id=discount_id)
    if not existing:
        raise DiscountNotFoundError(discount_id=discount_id)
    ...
    discount = await discounts_repository.update_discount(
...
async def delete_discount(*, session: AsyncSession, discount_id: int) -> None:
    ...
    deleted = await discounts_repository.delete_discount(session=session, discount_id=discount_id)
    if not deleted:
        raise DiscountNotFoundError(discount_id=discount_id)
...
async def enrich_products(
    *, session: AsyncSession, products: Sequence[Product]
    ...
    product_discounts = await discounts_repository.get_active_for_products(session=session, product_ids=product_ids)
    ...
    category_discounts = await discounts_repository.get_active_for_category_ids(
