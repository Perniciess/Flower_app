...
from app.modules.pickup_points import repository
from app.modules.pickup_points.model import PickupPoint
from app.modules.pickup_points.schema import PickupPointCreate, PickupPointResponse, PickupPointUpdate
...
async def create_pickup_point(session: AsyncSession, data: PickupPointCreate) -> PickupPointResponse:
    ...
    """
    pickup_point = await repository.create_pickup_point(session, data)
...
async def get_pickup_point_by_id(session: AsyncSession, pickup_point_id: int) -> PickupPointResponse:
    ...
    """
    pickup_point = await repository.get_pickup_point_by_id(session, pickup_point_id)
    if not pickup_point:
...
async def get_all_active_pickup_points(session: AsyncSession) -> Sequence[PickupPointResponse]:
    ...
    """
    pickup_points = await repository.get_all_active_pickup_points(session)
...
async def get_all_pickup_points(session: AsyncSession) -> Page[PickupPointResponse]:
    ...
    """
    query = repository.get_all_pickup_points_query()
...
async def update_pickup_point(
    ...
    updated_pickup_point = await repository.update_pickup_point(session, pickup_point, data)
...
async def delete_pickup_point(session: AsyncSession, pickup_point_id: int) -> None:
    ...
    await repository.delete_pickup_point(session, pickup_point_id)
