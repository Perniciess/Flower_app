...
async def create_product(*, session: AsyncSession, product_data: Mapping[str, Any]) -> Product:
    statement = Product(
...
async def get_product(*, session: AsyncSession, product_id: int) -> Product | None:
    statement = select(Product).where(Product.id == product_id).options(selectinload(Product.images))
...
async def update_product(*, session: AsyncSession, product_id: int, product_data: Mapping[str, Any]) -> Product | None:
    statement = update(Product).where(Product.id == product_id).values(**product_data).returning(Product)
