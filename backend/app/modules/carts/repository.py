...
async def get_cart_by_user_id(*, session: AsyncSession, user_id: int) -> Cart | None:
    cart = (
        select(Cart)
        .options(selectinload(Cart.cart_item))
...
async def get_cart_by_id(*, session: AsyncSession, cart_id: int) -> Cart | None:
    statement = (
        select(Cart).options(selectinload(Cart.cart_item)).where(Cart.id == cart_id)
    )
...
async def create_cart_item(
    *,
    ...
    cart_item = CartItem(
        cart_id=cart_id, flower_id=flower_id, quantity=quantity, price=price
    )
...
async def get_cart_item(
    *, session: AsyncSession, cart_id: int, flower_id: int
) -> CartItem | None:
    statement = select(CartItem).where(
        CartItem.cart_id == cart_id, CartItem.flower_id == flower_id
    )
...
async def delete_cart_item(*, session: AsyncSession, cart_item_id: int) -> bool:
    statement = (
        delete(CartItem).where(CartItem.id == cart_item_id).returning(CartItem.id)
    )
    result = await session.execute(statement)
    return result.scalar_one_or_none() is not None
